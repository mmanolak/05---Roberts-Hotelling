...
R0 <- 1000; pT <- 10; a <- 10; b <- a/pT

# Define the two different interest rates
r_base <- 0.05  # The original, baseline interest rate
r_shock <- 0.10 # The new, higher interest rate (the shock)
# NOTE: You can set r_shock to 0.50 to replicate your exaggerated test case.
...
# 2. The Corrected Comparative Plotting Function
...
create_comparative_plot <- function(time_base, price_base, quantity_base, stock_base,
  ...
  # Plot 1: Demand Curve (Top-Left) - CORRECTED LOGIC
  par(mar = c(0, 5, 5, 0))
  # First, create the data for the full, static demand curve
  p_demand <- seq(0, pT, length.out = 100)
  q_demand <- d(p_demand)
  # Plot the single, solid demand curve. There is no dotted line here.
  plot(q_demand, p_demand, type = "l", lwd = 2,
       xaxs = "i", yaxs = "i",
       xlim = rev(range(q_demand)),
       ylim = c(0, max(p_demand)),
  ...
  plot(time_base, price_base, type = "l", lwd = 2,
       ...
       xaxt = "n", yaxt = "n", xlab = "", ylab = "")
  lines(time_shock, price_shock, lwd = 2, lty = "dotted") # Overlay shocked path
  ...
  plot(stock_base, time_base, type = "l", lwd = 2,
       ...
       xaxt = "n", yaxt = "n", xlab = "", ylab = "")
  lines(stock_shock, time_shock, lwd = 2, lty = "dotted") # Overlay shocked path
  ...
  plot(time_base, time_base, type = "l", lwd = 2, lty = "solid",
       ...
       ylim = rev(range(c(time_base, time_shock))),
       xaxt = "n", yaxt = "n", xlab = "", ylab = "")
  lines(time_shock, time_shock, lwd = 2, lty = "dotted") # Overlay shocked path
